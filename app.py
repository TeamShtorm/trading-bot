    ...
    if field == "asset":
        ...
        try:
            ...
            if trade and trade['result'] != "BU":
                new_pnl = (trade['exit_price'] - val) * trade['volume'] if trade['direction'] == "LONG" else (val - trade['exit_price']) * trade['volume']
        ...
        try:
            ...
            trade = get_trade_by_id(trade_id, msg.from_user.id)
            if trade and trade['result'] != "BU":
                new_pnl = (val - trade['entry_price']) * trade['volume'] if trade['direction'] == "LONG" else (trade['entry_price'] - val) * trade['volume']
                update_trade_field(trade_id, msg.from_user.id, "pnl", new_pnl)
        except:
            await msg.answer(get_text(lang, "error_number"))
            return
        ...
        try:
            ...
            trade = get_trade_by_id(trade_id, msg.from_user.id)
            if trade and trade['result'] != "BU":
                new_pnl = (trade['exit_price'] - trade['entry_price']) * val if trade['direction'] == "LONG" else (trade['entry_price'] - trade['exit_price']) * val
                update_trade_field(trade_id, msg.from_user.id, "pnl", new_pnl)
        except:
            await msg.answer(get_text(lang, "error_number"))
            return
...
async def main():
    ...
    print("✅ Бот успешно запущен! Все функции работают.")
